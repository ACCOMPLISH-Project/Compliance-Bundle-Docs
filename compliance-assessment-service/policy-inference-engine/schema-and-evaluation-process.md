...
5. **Build field metadata**
    - For each field:
        - `key`, `type`, `required`, `requiredWhen`, `possibleValues`
        - `required` is true only for unconditional inputs, such as the `actions` selector.
        - `requiredWhen[]` contains simple request-field conditions. A field is required when any condition matches.
        - `provenance`:
            - `x-operands`
            - `x-policyIds`
            - `x-appliesTo`
